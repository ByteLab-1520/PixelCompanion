...
- Wraps long bilingual Advanced Settings labels so they never overlap their input controls.
- Holds the dedicated falling frame for the complete fall instead of allowing the idle animation timer to flash the default frame.
- Extends Yaroro's X-eyed stunned landing from 0.9 seconds to 2.5 seconds.
- Migrates the legacy saved full-screen Hide default to edge waiting once.
...
- Keeps Preview, Save, and Close available in a dedicated footer.
- Stops window-surface synchronization while the companion is being dragged, so a pet on a title bar no longer snaps back to the window.
- Refreshes available surfaces on release and lands on the nearest valid window or desktop surface.
