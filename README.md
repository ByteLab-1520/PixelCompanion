...
```text
┌──────────────────────────────────────────┐
│   PIXEL COMPANION FOR YARORO  v0.4.4     │
│          화면 한쪽에서, 조용히 함께       │
└──────────────────────────────────────────┘
```
...
[![Version](https://img.shields.io/badge/version-0.4.4-6f7cff?style=flat-square)](Directory.Build.props)
...
| 드래그와 착지 | 마우스로 들어 옮길 수 있고, 놓으면 가까운 창이나 바탕화면에 착지합니다. |
| 간단한 돌보기 | 밥 먹기, 놀아주기, 잠자기 동작을 사용할 수 있습니다. |
...
야로로의 기본 동작은 정면, 뒷모습, 걷기 1·2·3, 밥 먹기 1·2, 잠자기 1·2로 구성되어 있습니다. 사람 캐릭터에 맞춰 사료가 아니라 밥과 반찬을 먹습니다.
...
<p align="center">
  <img src="docs/media/yaroro-character-settings-v0.4.4.png" alt="야로로 기본 이미지가 표시된 캐릭터 설정 화면" width="900">
</p>
...
- 떨어지는 도중 다시 잡으면 이전 낙하와 착지 작업을 즉시 취소합니다.
- 여러 번 연속으로 잡았다 놓아도 가장 최근 착지만 캐릭터 위치를 바꿉니다.
- 저장하지 않은 대사를 닫을 때 나오는 확인창이 편집창 뒤에 숨지 않습니다.
- 캐릭터 설정창에서 사용자 이미지가 없으면 야로로 기본 동작 이미지를 보여 줍니다.
- `제거`를 누르면 빈칸이 아니라 기본 제공 이미지로 돌아갑니다.
- 밥 먹기 2와 잠자기 1 이미지 오른쪽에 남아 있던 흰색 조각을 제거했습니다.
- 야로로의 한국어 기본 인사를 `안녕? 야로로대장이야` 하나로 정리했습니다.
- 이전 버전의 수정하지 않은 기본 문장은 정리하되 사용자가 작성한 대사는 보존합니다.

자세한 기록은 [v0.4.4 변경사항](docs/releases/v0.4.4.md)에서 볼 수 있습니다.
...
| `v0.5.0` | 배고픔·청결도·행복도·피로도·친밀도와 타이머 |
